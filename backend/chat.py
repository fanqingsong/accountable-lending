...
from backend.retrieve import DECISION_CATEGORIES, retrieve
...
def generate_answer(query: str, payload: Dict[str, Any], complete: Optional[CompleteFn] = None) -> Dict[str, str]:
    ...
    source = "extractive"
    has_evidence = bool(payload.get("decisions") or payload.get("chunks"))
    if complete is not None and context and has_evidence:
