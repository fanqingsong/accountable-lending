...
QUERY_ALIASES = {
    ...
    "sunrise": "Sunrise Coffee Roasters",
    "harbor": "Harbor Bakehouse",
}
...
def hinted_applications(query: str) -> List[str]:
    expanded = expand_query(query).lower()
    hints = []
    for term, app_id in (("sunrise", "sunrise-coffee"), ("harbor", "harbor-bakery")):
        if term in expanded:
            hints.append(app_id)
    return hints
...
def related_decisions(graph, application_ids: List[str], mapping: Dict[str, str] | None = None) -> Dict[str, Any]:
    ...
    caused = [
        ...
        for edge in edges
        if str(edge.get("type") or "").upper() == "CAUSED"
    ]
...
def retrieve(graph, query: str, store=None, limit: int = 8) -> Dict[str, Any]:
    ...
    hinted = hinted_applications(query)
    app_ids = list(hinted)
    if not app_ids:
        for hit in chunks:
